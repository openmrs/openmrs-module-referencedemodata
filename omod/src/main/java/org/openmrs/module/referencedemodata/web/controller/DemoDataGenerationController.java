...
public class DemoDataGenerationController extends BaseRestController {
	...
	@RequestMapping(value = ReferenceDemoDataConstants.GENERATE_DEMO_DATA_URI, method = RequestMethod.GET, produces = "application/json")
	@ResponseBody()
	public String generateDemoData(HttpServletRequest request) throws Exception {
		
		String numberOfDemoPatients = request.getParameter(NUMBER_OF_DEMO_PATIENTS_PARAMETER);
		...
		if (Context.getPatientService().getAllPatients().size() < Integer.parseInt(numberOfDemoPatients)) {
			int remainingNumberOfPatientsToGenerate =  Integer.parseInt(numberOfDemoPatients) - Context.getPatientService().getAllPatients().size();
